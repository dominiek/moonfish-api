...
module.exports = function validate(schemas, options) {
  ...
  return async function Validate(ctx, next) {
    Object.keys(schemas).forEach(key => {
      ...
      Joi.validate(requestItem, schema, {
        ...defaultOptions,
        ...options
      }, (err, validatedItem) => {
        if (err) {
          err.details = err.details.map(detail => { //eslint-disable-line
            return {
              ...detail,
              meta: {
        ...
        const unverifiedParams = Object.keys(validatedItem || {})
          .filter(param => !Joi.reach(jSchema, param));
        ...
        if (key === 'query') {
          ...
          const src = {
            ...
            get query() {
              ...
              return (this._querycache && this._querycache[this.querystring]
                ? validatedItem
                : originalQueryGetter
              );
            }
